...
- The marketing guide is the SOP — keep it in sync with every change (`build_guide.py`, rebuild, re-send the PDF). `marketing/` is gitignored. **Its version FOLLOWS THE ENGINE** (read live from `admin/js/version.js`): an engine build is `v<N>` (e.g. v63); a rebuild that is ONLY a manual/content change adds a letter — `v63a`, `v63b`… — reset to plain `v<N+1>` on the next engine sync. `build_guide.py` reads the engine number itself; just set the PDF-only `LETTER` (currently `""` — the engine is v63, so the manual is plain v63).
...
| Change history | — | `CHANGELOG.md` (localized v54→v63) → root `changelog.pdf` (via gitignored `marketing/build_changelog.py`), linked from More → Full change history |
...
- Full test suite green: `node --test test/*.test.js` → **501 tests, all passing** (470 pre-v63-sync; 436 pre-v61-sync).
