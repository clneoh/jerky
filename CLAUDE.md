...
- The marketing guide is the SOP — keep it in sync with every change (`build_guide.py`, rebuild, re-send the PDF). `marketing/` is gitignored. **Its version FOLLOWS THE ENGINE** (read live from `admin/js/version.js`): an engine build is `v<N>` (e.g. v67); a rebuild that is ONLY a manual/content change adds a letter — `v67a`, `v67b`… — reset to plain `v<N+1>` on the next engine sync. `build_guide.py` reads the engine number itself; just set the PDF-only `LETTER` (currently `""` — the engine is v67, so the manual is plain v67).
...
| Change history | — | `CHANGELOG.md` (localized v54→v67) → root `changelog.pdf` (via gitignored `marketing/build_changelog.py`), linked from More → Full change history |
...
- Full test suite green: `node --test test/*.test.js` → **568 tests, all passing** (567 pre-v67-sync; 528 pre-v66-sync; 501 pre-v65-sync; 470 pre-v63-sync; 436 pre-v61-sync).
