...
- The marketing guide is the SOP — keep it in sync with every change (`build_guide.py`, bump VERSION, rebuild, re-send the PDF). `marketing/` is gitignored.
...
| Postage | `settings.storefront.postageRM` | flat per-order fee, on the WhatsApp To-pay line, never on the storefront |
